...
main = do
  ...
  let input = map (\l -> (head l, read $ tail l)) $ lines contents :: [(Char, Int)]
  print input
  print $ part1 input
  print $ part2 input
...
solve :: [(Char, Int)] -> ((Point, Point) -> (Char, Int) -> (Point, Point)) -> Point -> Int
solve input applier start = abs (fst final) + abs (snd final)
  where 
    final :: (Int, Int)
    final = snd $ foldl applier (start, (0, 0)) input

p1apply :: (Point, Point) -> (Char, Int) -> (Point, Point)
...
p2apply :: (Point, Point) -> (Char, Int) -> (Point, Point)
